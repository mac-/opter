...
module.exports = function (options, appVersion, c) {
	...
	for (optName in options) {
		if (options.hasOwnProperty(optName)) {
			option = options[optName];
			config[optName] = commander[optName] || process.env[optName] || configFile[optName] || option.defaultValue;
			if (option.hasOwnProperty('type')) {
				switch (option.type) {

				case Boolean:
					config[optName] = (config[optName] === "true");
					break;

				case Number:
					config[optName] = parseInt(config[optName], 10);
					break;

				case Date:
					// test string to see if it's an all-numeric value, and if so, parse it
					config[optName] = (/^\d+$/.test(config[optName], 10)) ? parseInt(config[optName], 10) : config[optName];
					config[optName] = new Date(config[optName]);
					break;

				default:
					config[optName] = config[optName];
				}
